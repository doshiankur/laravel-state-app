...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li><a href="#"> Membership Users </a></li>
...
<style type="text/css">
    ...
    <div class="box box-info">
          ...
          <?php }?>    -->       
                    ...
                    {!! Form::label('email', "User IDProof: ", ['class' => 'col-sm-2']) !!}
                      <div class="col-sm-8">
                       <!--   <a target="_blank" 
                         ...
                         <a download={{ $memebrsip_getdata->strIDProof}}
                         href="data:application/pdf;base64,{{ base64_encode($member_data['strIDProof'])}}">
                         {{ $memebrsip_getdata->strIDProof}}</a>   
                    </div>
                </div>  
                ...
                <div class="form-group">
                    {!! Form::label('email', "User Photo: ", ['class' => 'col-sm-2']) !!}
                      <div class="col-sm-8">
                        <a download={{ $memebrsip_getdata->strPhoto}}
                         href="data:image/jpg;base64,{{ base64_encode($member_data['strPhoto'])}}">
                ...
                <div class="form-group">
                    {!! Form::label('email', "User Signed: ", ['class' => 'col-sm-2']) !!}
                      <div class="col-sm-8">
                         ...
                         <a download={{ $memebrsip_getdata->strSigned}}
                         href="data:image/jpg;base64,{{ base64_encode($member_data['strSigned'])}}">
                         {{ $memebrsip_getdata->strSigned}}</a>  
                    </div>
                </div> 
                  ...
                  <div class="form-group">
                    {!! Form::label('email', "Final Submit Copy: ", ['class' => 'col-sm-2']) !!}
                      <div class="col-sm-8">
                         ...
                         <a download={{ $memebrsip_getdata->strFinalScan}}
                         href="data:application/pdf;base64,{{ base64_encode($member_data['strFinalScan'])}}">
                         {{ $memebrsip_getdata->strFinalScan}}</a> 
                      </div>   
                </div>
        ...
        <?php if (!$membership_query->isEmpty()){?>
             <h1 class="form-control">Query History</h1>
                <div class="box-body">
                    <table id="example" class="display" style="width:100%" border="1">
                        <thead>
                        <tr><th>Sr.No</th>
                            <th>Query Against Application</th>
                            <th>Query status</th>
                            <th>Query Date</th>
                        </tr>
                        </thead>
                        <tbody>
                            <?php $i=1;?>
                       @foreach($membership_query as $item)
         ...
         <?php if($memebrsip_getdata->enmApplicationStatus==2 || $memebrsip_getdata->enmApplicationStatus==4){?>
          ...
          <?php }elseif($memebrsip_getdata->enmApplicationStatus==5){?>
               
            <div class="box-footer">
                <div class="form-group">
                    <div class="col-md-offset-4 col-md-4">                   
                        <a href="{{ url('/webpanel/membership/status/'.$memebrsip_getdata->id)}}" title="Approved Application" class="btn btn-success"><i class="fa " aria-hidden="true"></i>Approved</a>
                    </div>
                </div>
            </div>
          <?php }?>
